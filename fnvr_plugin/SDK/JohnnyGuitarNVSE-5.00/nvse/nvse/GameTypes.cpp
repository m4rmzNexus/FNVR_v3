...
void BSString::Init(UInt32 bufSize) {
...
void BSString::AppendChar(char toAppend) {
	if (m_bufLen == m_dataLen) {
		m_bufLen++;
		char* newStr = (char*)GameHeapAlloc(m_bufLen + 1);
		if (m_data) {
...
void BSString::InsertChar(char toInsert, UInt32 index) {
...
void BSString::EraseAt(UInt32 index) {
